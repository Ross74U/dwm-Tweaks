...
static size_t nclient(Monitor *m);
static void toggle_auto_nmaster(const Arg *arg);
static void auto_nmaster();
...
createmon(void)
{
	...
	m->mfact = mfact;
	m->nmaster = nmaster;
	m->auto_nmaster = auto_nmaster_enable;
	m->showbar = showbar;
	m->topbar = topbar;
	m->gappx = gappx;
	m->lt[0] = &layouts[0];
...
format_mode(Monitor *m, char *buf, size_t bufsz) {
    if (m->auto_nmaster == 1) {
      ...
      snprintf(buf, bufsz, "%s master %d", m->ltsymbol, m->nmaster);
    }
}
...
drawbar(Monitor *m)
{
  ...
  // custom format string for nmaster + ltsymbol
  char mode_buf[64];
  format_mode(m, mode_buf, sizeof mode_buf);

	w = TEXTW(mode_buf);
	drw_setscheme(drw, scheme[SchemeNorm]);
	x = drw_text(drw, x, 0, w, bh, lrpad / 2, mode_buf, 0);
...
void auto_nmaster(Monitor *m) {
    ...
    int nmaster;
    size_t n = nclient(m);
    size_t mapping_len = sizeof (mastermapping) / sizeof (mastermapping[0]);
    if (n >= mapping_len) {
      nmaster = n / 2; 
    } else {
      nmaster = mastermapping[n];
    }
    m->nmaster = nmaster;
    drawbar(m);
}
